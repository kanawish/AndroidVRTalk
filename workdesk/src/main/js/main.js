...
/* TODO: DEMO MINECRAFTY LANDSCAPE
 */
...
var planeInstanced = m.buildInstancedFromArray( [
    ...
    [[-1.6, -2.5, -12], [rot*1.3, 0, 0], [1, 1, 1], [1.0, 0.3, 0.0, 1], [1,0,0,0]]
] );
...
/*
    TODO: Implement
    Here, we'll define a grid of points, pass the randomized height of each as a parameter.
    Not efficient, but demonstrates how flexible the approach can be.
 */
for( var x = 0 ; x < 50 ; x++ ) {
    for (var y = 0; y < 50; y++) {
        var div = 10 ;
        // 2 triangles per x/y
        var h1 = p.noise.simplex2(x/div,y/div);
        var h2 = p.noise.simplex2((x+1)/div,y/div);
        var h3 = p.noise.simplex2(x/div,(y+1)/div);
        var h4 = p.noise.simplex2((x+1)/div,(y+1)/div);

        // TODO: Finish Landscape mesh generation
    }
}
